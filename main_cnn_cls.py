...
if __name__ == '__main__':
    ...
    model_cls = 'ResNet50_AIC20_veri_classifier_latest.pth.tar'  # triplet loss

    model_cls_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models/' + model_cls)

    # net_cls = joined_network.joined_network('ResNet50', False)
    net = net_id_classifier.net_id_classifier('ResNet50', 903)
    weights = torch.load(model_cls_path)['state_dict']
    ...
    for st in set:
        ...
        # Results file
        file_results = os.path.join(results_dir, s, 's02_tripletloss.txt')
            ...
            for f in range(1,  mtmc.max_frame[s] + 1):
                ...
                if num_det_f != 0:
                    ...
                    if (sum(sum(dist_flag)) != 0) and ((np.unique(sct_f['id_cam'])).size > 1):
                        ...
                        dist_logits_sig = np.zeros((num_det_f, num_det_f))

                        for b1 in range(num_det_f):
                            for b2 in range(num_det_f):
                                if (b1 != b2) & (b1 > b2):
                                    # print(b1)
                                    # print(b2)
                                    dist_logits_sig[b1, b2] = sigmoid_function(net_cls(sct_f['bbox'][b1].unsqueeze(0).cuda(), sct_f['bbox'][b2].unsqueeze(0).cuda())) # probability of being different cars
                                    dist_logits_sig[b2, b1] = dist_logits_sig[b1, b2]
                                        # association_matrix_logits[b1, b2] = 1
                                        # association_matrix_logits[b2, b1] = 1
                        ...
                        if feat.characteristic == 'distance':
                            restricted_dist_features, association_matrix = feat.apply_restrictions_logits(dist_spatial, dist_spatial, sct_f, mtmc.dist_th,feat.characteristic)
                            ...
                            restricted_dist_features, association_matrix = feat.apply_restrictions_logits(dist_logits_sig,  dist_spatial, sct_f, mtmc.dist_th,feat.characteristic)
                            ...
                            idx, optimal_clusters = clust.compute_clusters(restricted_dist_features, association_matrix)
                             ...
                             # Clustering
                            restricted_dist_features, association_matrix = feat.apply_restrictions_logits(dist_logits_sig.copy(), dist_spatial, sct_f,  mtmc.dist_th,feat.characteristic)
                            idx, optimal_clusters = clust.compute_clusters(restricted_dist_features, association_matrix)
